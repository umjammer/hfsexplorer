...
//    public int length() { return occupiedSize(); }
    ...
    public byte getCkrKeyLen() {
        return Util.readByteBE(ckrKeyLen);
    }

    public byte getCkrResrv1() {
        return Util.readByteBE(ckrResrv1);
    }

    public int getCkrParID() {
        return Util.readIntBE(ckrParID);
    }

    public byte getCkrCNameLen() {
        return Util.readByteBE(ckrCNameLen);
    }
    ...
    public byte[] getBytes() {
        ...
        System.arraycopy(ckrKeyLen, 0, result, offset, ckrKeyLen.length);
        offset += ckrKeyLen.length;
        System.arraycopy(ckrResrv1, 0, result, offset, ckrResrv1.length);
        offset += ckrResrv1.length;
        System.arraycopy(ckrParID, 0, result, offset, ckrParID.length);
        offset += ckrParID.length;
        System.arraycopy(ckrCNameLen, 0, result, offset, ckrCNameLen.length);
        offset += ckrCNameLen.length;
        System.arraycopy(ckrCName, 0, result, offset, ckrCName.length);
        offset += ckrCName.length;
        System.arraycopy(ckrPad, 0, result, offset, ckrPad.length);
        offset += ckrPad.length;
        return result;
    }
