...
public class HFSUniStr255 implements StructElements, PrintableStruct {
    ...
    public HFSUniStr255(byte[] data, int offset, int length) {
        if (offset > data.length || length < 2) {
            throw new HFSNotEnoughDataException(false, data.length, offset, 2, getClass());
        }
        ...
        if (stringLength > 255) {
            ...
            throw new HFSNotEnoughDataException(true, data.length, offset, 2 + stringLength, getClass());
        }
