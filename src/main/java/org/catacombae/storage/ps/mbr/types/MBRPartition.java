...
public class MBRPartition implements Partition {
    ...
    public byte[] getFirstSector() {
        return Util.createCopy(firstSector);
    }

    public byte getPartitionType() {
        return Util.readByteLE(partitionType);
    }
    ...
    public byte[] getLastSector() {
        return Util.createCopy(lastSector);
    }

    public int getLBAFirstSector() {
        return Util.readIntLE(lbaFirstSector);
    }

    public int getLBAPartitionLength() {
        return Util.readIntLE(lbaPartitionLength);
    }
    ...
    public byte[] getBytes() {
        ...
        int i = 0;
        System.arraycopy(status, 0, result, i, status.length);
        i += status.length;
        System.arraycopy(firstSector, 0, result, i, firstSector.length);
        i += firstSector.length;
        System.arraycopy(partitionType, 0, result, i, partitionType.length);
        i += partitionType.length;
        System.arraycopy(lastSector, 0, result, i, lastSector.length);
        i += lastSector.length;
        System.arraycopy(lbaFirstSector, 0, result, i, lbaFirstSector.length);
        i += lbaFirstSector.length;
        System.arraycopy(lbaPartitionLength, 0, result, i, lbaPartitionLength.length);
        i += lbaPartitionLength.length;
