...
public class DriverDescriptorEntry {
    ...
    public int getDdBlock() {
        return Util.readIntBE(ddBlock);
    }

    public short getDdSize() {
        return Util.readShortBE(ddSize);
    }

    public short getDdType() {
        return Util.readShortBE(ddType);
    }

    public byte[] getData() {
        byte[] result = new byte[length()];
        int offset = 0;
        System.arraycopy(ddBlock, 0, result, offset, ddBlock.length);
        offset += ddBlock.length;
        System.arraycopy(ddSize, 0, result, offset, ddSize.length);
        offset += ddSize.length;
        System.arraycopy(ddType, 0, result, offset, ddType.length);
        offset += ddType.length;
        if (offset != length())
