...
public class DSStoreInfo {
    ...
    private static void printAlias(byte[] data, int offset, int size, String insetString) {
        ...
        for (int j = AliasHeader.STRUCTSIZE; j < size; ) {
            short entryTypeValue = Util.readShortBE(data, offset + j);
            j += 2;

            int entrySize = Util.unsign(Util.readShortBE(data, offset + j));
            j += 2;

            EntryType entryType = EntryType.lookupByValue(entryTypeValue);
            ...
            switch (entryType) {
                ...
                case EntryTypeAppleRemoteAccessDialupInfo:
                    System.out.println("AppleRemoteAccess dialup info:");
                    break;
                ...
                default:
                    System.out.println("Unknown entry type " + entryTypeValue + ":");
                    break;
            }

            System.out.println(insetString + "        Size: " +
                    entrySize + " (0x" + Util.toHexStringBE((short) entrySize) + ")");
            ...
            switch (entryType) {
                ...
                case EntryTypeDirectoryNameUnicode:
                    System.out.println(insetString + "        Name " +
                            "length: " + Util.readShortBE(data, offset + j));
                    System.out.println(insetString + "        Name: " +
    ...
    private static void printBinaryPlistValue(BinaryPlist.Entry entry, String insetString) {
        ...
        if (entry == null) {
            ...
            System.out.println(insetString + "  " + e.getValue());
        } else if (entry instanceof BinaryPlist.ArrayEntry || entry instanceof BinaryPlist.SetEntry) {
            ...
            System.out.println(insetString +
                    ((marker & 0xF0) == 0xC0 ? "Set" : "Array") + " (" + entries.size() + " elements):");
            ...
            System.out.println(insetString + "<uid: 0x" + Util.toHexStringBE(entry.getMarker()) + ">");
        } else {
            System.out.println(insetString + "<unknown: 0x" + Util.toHexStringBE(entry.getMarker()) + ">");
        }
    }

    private static void printBinaryPlist(byte[] dsStoreData, int curOffset, int blobSize, String insetString) {
        BinaryPlist plist = new BinaryPlist(dsStoreData, curOffset, blobSize);
...
//        plist.getFooter().printFields(System.out, insetString);
    ...
    private static void printTreeBlockRecursive(
        ...
        int blockMode = Util.readIntBE(dsStoreData, 4 + (int) dataBlockOffset + 0);
        int recordCount = Util.readIntBE(dsStoreData, 4 + (int) dataBlockOffset + 4);
        System.out.println(insetString + "    block mode: 0x" + Util.toHexStringBE(blockMode));
        System.out.println(insetString + "    record count: 0x" + Util.toHexStringBE(recordCount));
        ...
        for (int i = 0; i < recordCount; ++i) {
            ...
            if (blockMode != 0x0) {
                ...
                // the record so that we can descend further into the tree.
                childNodeBlockID = Util.readIntBE(dsStoreData, curOffset);
                System.out.println(insetString + "      Child node block ID: " + childNodeBlockID);
                curOffset += 4;
            }

            int filenameLength = Util.readIntBE(dsStoreData, curOffset);
            System.out.println(insetString + "      Filename length: " + filenameLength);
            curOffset += 4;

            String filename = Util.readString(dsStoreData, curOffset, filenameLength * 2, "UTF-16BE");
            ...
            System.out.println(insetString + "      Structure ID: " + Util.toASCIIString(structID) + " " +
                    "(0x" + Util.toHexStringBE(structID) + ")");
            curOffset += 4;

            int structType = Util.readIntBE(dsStoreData, curOffset);
            System.out.println(insetString + "      Structure type: " + Util.toASCIIString(structType) + " " +
            ...
            if (Util.toASCIIString(structType).equals("long")) {
                ...
                System.out.println(insetString + "      Padding: " + padding + " / 0x" + Util.toHexStringBE(padding));
                curOffset += 2;
                ...
                System.out.println(insetString + "      Blob size: " + blobSize);
                curOffset += 4;

                System.out.println(insetString + "      Blob data:");
                hexDump(dsStoreData, curOffset, blobSize, insetString + "        ");
                ...
                if (Util.toASCIIString(structID).equals("pict")) {
                    ...
                    System.out.println(insetString + "      Finder background:");

                    int backgroundType = Util.readIntBE(dsStoreData, curOffset);
                    System.out.print(insetString + "        Type: '" + Util.toASCIIString(backgroundType) + "' ");
                    ...
                    if (Util.toASCIIString(backgroundType).equals("DefB")) {
                        ...
                        System.out.println(insetString + "        Reserved: " +
                                "0x" + Util.byteArrayToHexString(dsStoreData, curOffset + 4, 8));
                    } else if (Util.toASCIIString(backgroundType).equals("ClrB")) {
                        System.out.println("(Solid color)");

                        short red = Util.readShortBE(dsStoreData, curOffset + 4);
                        short green = Util.readShortBE(dsStoreData, curOffset + 6);
                        short blue = Util.readShortBE(dsStoreData, curOffset + 8);
                        short unknown = Util.readShortBE(dsStoreData, curOffset + 10);
                        System.out.println(insetString + "        Red: " + red);
                        System.out.println(insetString + "        Green: " + green);
                        System.out.println(insetString + "        Blue: " + blue);
                        System.out.println(insetString + "        Reserved / " +
                        ...
                        int pictBlobSize = Util.readIntBE(dsStoreData, curOffset + 4);
                        int unknown = Util.readIntBE(dsStoreData, curOffset + 8);
                        System.out.println(insetString + "        Blob size: " + pictBlobSize);
                        System.out.println(insetString + "        Reserved / " +
                                "unknown: 0x" + Util.toHexStringBE(unknown));
                    } else {
                        System.out.println(insetString + "        Type: Unknown ('" +
                                Util.toASCIIString(backgroundType) + "')");
                        System.out.println(insetString + "        Unknown / " +
                                "Reserved: 0x" + Util.byteArrayToHexString(dsStoreData, curOffset + 4, 8));
                    }
                } else if (Util.toASCIIString(structID).equals("Iloc") && blobSize == 16) {
                    //
                    // Location of file's icon within the Finder window.
                    //

                    System.out.println(insetString + "      Finder icon location:");

                    int x = Util.readIntBE(dsStoreData, curOffset);
                    int y = Util.readIntBE(dsStoreData, curOffset + 4);
                    System.out.println(insetString + "        Horizontal (X): " + x);
                    System.out.println(insetString + "        Vertical (Y): " + y);
                    System.out.println(insetString + "        Unknown / " +
                            "Reserved: 0x" + Util.byteArrayToHexString(dsStoreData, curOffset + 8, 8));
                } else if (Util.toASCIIString(structID).equals("bwsp")) {
                    System.out.println(insetString + "      Finder window properties:");

                    printBinaryPlist(dsStoreData, curOffset, blobSize, insetString + "        ");
                } else if (Util.toASCIIString(structID).equals("fwi0")) {
                    System.out.println(insetString + "      Finder window information:");
                    ...
                    System.out.println(insetString + "        Bottom: " + bottom);
                    ...
                    System.out.println(insetString + "        View: " +
                            viewLabel + " (" + Util.toASCIIString(view) + " / 0x" + Util.toHexStringBE(view) + ")");

                    System.out.println(insetString + "        Unknown / " +
                            "Reserved: 0x" + Util.byteArrayToHexString(dsStoreData, curOffset + 12, 4));
                } else if (Util.toASCIIString(structID).equals("icvo")) {
                    System.out.println(insetString + "      Finder icon view options:");
                    ...
                    if (Util.toASCIIString(magic).equals("icvo")) {
                        ...
                        iconViewSize = Util.readShortBE(dsStoreData, curOffset + 12);
                        keepArrangedBy = Util.readIntBE(dsStoreData, curOffset + 14);
                    } else if (Util.toASCIIString(magic).equals("icv4")) {
                        // 26-byte format
                        iconViewSize = Util.readShortBE(dsStoreData, curOffset + 4);
                        keepArrangedBy = Util.readIntBE(dsStoreData, curOffset + 6);
                        iconLabelPosition = Util.readIntBE(dsStoreData, curOffset + 10);
                        flags = Util.readByteArrayBE(dsStoreData, curOffset + 14, 12);
                    } else {
                        System.out.println(insetString + "        " +
                    ...
                    if (flags != null) {
                        System.out.println(insetString + "        Flags: 0x" + Util.byteArrayToHexString(flags));
                        if ((flags[1] & 0x1) != 0) {
                            System.out.println(insetString + "          Show item info");
                        }
                        if ((flags[11] & 0x1) != 0) {
                            System.out.println(insetString + "          Show icon preview");
                        }
                    }
                }
                ...
                // A long is a 4 byte (32-bit) big-endian integer.
                int value = Util.readIntBE(dsStoreData, curOffset);
                System.out.println(insetString + "      Value: '" + Util.toASCIIString(value) + "'");
                curOffset += 4;
            } else if (Util.toASCIIString(structType).equals("ustr")) {
                // A long is a 4 byte (32-bit) big-endian integer.
                int length = Util.readIntBE(dsStoreData, curOffset);
                System.out.println(insetString + "      String length: " + length + " / 0x" + Util.toHexStringBE(length));
                curOffset += 4;

                String string = Util.readString(dsStoreData, curOffset, length * 2, "UTF-16BE");
                ...
                // epoch).
                long value = Util.readLongBE(dsStoreData, curOffset);
                System.out.println(insetString + "      Timestamp: " + value + " / 0x" + Util.toHexStringBE(value));
                curOffset += 8;
            ...
            if (blockMode != 0x0) {
                ...
                // manner.
                System.out.println(insetString + "      Child node " + childNodeBlockID + ":");
                printTreeBlockRecursive(dsStoreData, rootBlock, inset + 1, childNodeBlockID);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        DSStoreRootBlock rootBlock = new DSStoreRootBlock(dsStoreData, h.getRawRootBlockOffset1());
        rootBlock.print(System.out, "");

        DSStoreTableOfContents toc = new DSStoreTableOfContents(dsStoreData,
                        h.getRawRootBlockOffset1() + rootBlock.maxSize());
        toc.print(System.out, "");

        DSStoreFreeList freeList = new DSStoreFreeList(dsStoreData, h.getRawRootBlockOffset1() +
        ...
        for (int i = 0; i < toc.getTocCount(); ++i) {
            ...
            long treeLocator = rootBlock.getOffsetList()[tocEntry.getRawTocValue()];
            ...
            DSStoreTreeBlock b = new DSStoreTreeBlock(dsStoreData, 4 + (int) treeOffset);
            b.print(System.out, "  ");

            printTreeBlockRecursive(dsStoreData, rootBlock, 0, b.getRawFirstDataBlockID());
        }
